...
class testDNSService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.srvRecord = [0, 0, self.port, self.addr]
    ...
    def helper_mock_dns_utils(self, dns):
        ...
        def getTXTRecordSet(name):
            txtRecord = MagicMock()
            txtRecord.__getitem__.return_value = self.txtRecord
    ...
    def test_initialise_txt_from_records(self):
        self.assertEqual(self.dut.txt, self.txtRecord)
    ...
    def test_initialise_addr(self):
        self.assertEqual(self.addr, self.dut.addr)
    ...
    def test_ttl_callback_update(self):
        self.port = 9000
        self.srvRecord = [0, 0, 9000, "hostname.example.com."]
        with patch('nmoscommon.mdns.dnsService.Timer') as timer:
            with patch('nmoscommon.mdns.dnsService.dnsUtils') as dns:
                self.helper_mock_dns_utils(dns)
                self.helper_mock_timer(timer)
    ...
    def test_remove_dnsListener_call(self):
        self.helper_remove_service()
        self.assertTrue(self.dut.dnsListener.remove_service.called)

    def test_remove_dnsListener_call_args(self):
        self.helper_remove_service()
        args, _ = self.dut.dnsListener.remove_service.call_args
